package coursera.datastruct.week1;

import java.util.*;
import java.io.*;

public class tree_height {
    class FastScanner {
		StringTokenizer tok = new StringTokenizer("");
		BufferedReader in;

		FastScanner() {
			in = new BufferedReader(new InputStreamReader(System.in));
		}

		String next() throws IOException {
			while (!tok.hasMoreElements())
				tok = new StringTokenizer(in.readLine());
			return tok.nextToken();
		}
		int nextInt() throws IOException {
			return Integer.parseInt(next());
		}
	}

	public class TreeHeight {
		int n;
		int parent[];
		Node[] nodes;
		
		void read() throws IOException {
			FastScanner in = new FastScanner();
			n = in.nextInt();
			parent = new int[n];
			for (int i = 0; i < n; i++) {
				parent[i] = in.nextInt();
			}
		}

		int computeHeight() {
           int root = -1;
           nodes = new Node[n];
           for(int i = 0; i < n; i++) {
        	   nodes[i] = new Node(i);
           }
           for(int child = 0; child < n; child++) {
        	   int parentIndex = parent[child];
        	   if(parentIndex == -1) {
        		   root = child;
        	   } else {
        		   nodes[parentIndex].children.add(nodes[child]);
        		   //nodes[child].parent = nodes[parentIndex];
        	   }
           }
           int height = getHeight(root);
           return height;
		}
		
		int getHeight(int start) {
			int height = 0;
			Queue<Node> toBeVisited = new LinkedList<>();
			toBeVisited.add(nodes[start]);
			Node n;
			int current_nodes = 1;
			int next_nodes = 0;
			while(!toBeVisited.isEmpty()) {
				for(int i = 0; i < current_nodes; i++) {
					n = toBeVisited.poll();
					toBeVisited.addAll(n.children);
					next_nodes += n.children.size();
				}
				height++;
				current_nodes = next_nodes;
				next_nodes = 0;
			}
			return height;
		}
	}
	
	class Node {
		//Node parent;
		int key;
		List<Node> children = new LinkedList<>();
		
		public Node(int key) {
			this.key = key;
		}
		
		@Override
		public String toString() {
			return "{key="+key+", children="+children+"}";
		}
	}

	static public void main(String[] args) throws IOException {
            new Thread(null, new Runnable() {
                    public void run() {
                        try {
                            new tree_height().run();
                        } catch (IOException e) {
                        }
                    }
                }, "1", 1 << 26).start();
	}
	public void run() throws IOException {
		TreeHeight tree = new TreeHeight();
		tree.read();
		System.out.println(tree.computeHeight());
	}
}
